...
//
    ...
    public class MidiFile_2
    {
        ...
        public MidiFile_2(Stream stream)
            : this(Reader.ReadAllBytesFromStream(stream))
        {
        }

        public MidiFile_2(string path)
            : this(File.ReadAllBytes(path))
        {
        }

        public MidiFile_2(byte[] data)
        {
            var position = 0;
            ...
            while (position < trackEnd)
            {
                ...
                if ((status & 0xF0) != 0xF0)
                {
                    ...
                    track.MidiEvents.Add(
                        new MidiEvent_2 { Time = time, Type = eventType, Arg1 = channel, Arg2 = data1, Arg3 = data2 });
                }
                else
                {
                    if (status == 0xFF)
    ...
    public class MidiTrack
    {
        ...
        public List<MidiEvent_2> MidiEvents = new List<MidiEvent_2>();

        public List<TextEvent> TextEvents = new List<TextEvent>();
    }

    public struct MidiEvent_2
{
        public int Time;
